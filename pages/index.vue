...
const validateWordInput = () => {
    if (localFormData.filterBy === "comment" && localFormData.findByWord) {
        ...
        if (word.length === 0 && !localFormData.hasImage) {
            isWordCorrect.value = false;
            wordErrorText.value =
                "Введите слово/фразу или выберите изображение";
            return;
        }
...
console.log("result: ", formDataStore.getLink);
</script>
...
<template>
    ...
    >
        <div>
            <fieldset>
                <legend class="title">
                    Ссылка на пост или записку в кострище:
                </legend>
                <input
                    id="link"
            ...
            <fieldset>
                ...
                <div v-if="localFormData.filterBy === 'comment'" class="mt-4">
                    <input
                        ...
                        type="checkbox"
                    />
                    <label class="pl-2" for="findByWord"
                        >Выбирать по содержанию</label
                    >
                    ...
                    <div v-if="localFormData.findByWord">
                        ...
                        <div class="pt-4">
                            <input
                                ...
                                @blur="validateWordInput"
                            />
                            <label class="pl-2" for="imgInclude"
                                >Содержит изображение</label
                            >
                        </div>
                    </div>
                </div>
