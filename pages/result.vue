...
<template>
  ...
  >
    ...
    <div v-if="isLoading" role="status">
      <svg
        aria-hidden="true"
        class="w-15 h-15 text-gray-200 animate-spin dark:text-gray-600 fill-(--color-steam-key)"
        fill="none"
        viewBox="0 0 100 101"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          class="fill-(--color-blue-100)"
    ...
    <div v-else>
      <div class="flex flex-col items-center gap-4 pb-4">
        <h1 class="title text-center">
          ...
          {{
            formDataStore.getFilterByLike && formDataStore.getFilterByComment
              ? "лапок с комментами"
              : formDataStore.getFilterByLike
              ? "лапок"
              : formDataStore.getFilterByComment
              ? "комментов"
              : "участников"
          }}
          по ссылке:
        ...
        <a
          ...
          class="text-(--color-blue-600) text-center no-underline hover:underline transition duration-200"
        >
          {{ formDataStore.getLink }}
        </a>
        <h2>
          Количество участников:
          <span class="text-(--color-blue-600)">{{
      ...
      <div
        ...
        "
      >
        <h2 class="text-center text-blue-900">Нет подходящих участников =(</h2>
      </div>

      <div
        v-else-if="
          formDataStore.getWinnersCount > resultStore.getParticipants.length
        "
      >
        <h2 class="text-center text-red-500">
          Максимальное количество победителей не может превышать число
          участников ( {{ resultStore.getParticipants.length }} )
        </h2>
      </div>

      <div
        v-else
        class="flex flex-col items-center gap-4 border-t border-blue-500"
      >
        <h2 class="title pt-4">
          {{
        ...
        <div class="flex flex-col gap-4">
          ...
          >
            <a
              ...
              class="text-(--color-blue-600) flex items-center gap-2.5"
            >
              <img
